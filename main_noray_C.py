...
generator = [Generator(opt.img_size, opt.latent_dim, opt.channels).apply(weights_init_normal) for i in range(pop_size)]
base_model = Base(opt.channels)
classifier = Classifier(base_model,opt.img_size, classfier)
discriminator = Discriminator(base_model, opt.img_size)
...
class Worker(object):
    ...
    def do_rollouts(self, z, parent_param, generator, discriminator, classifier, j, step):
        ...
        if random.random() > get_decay_prop(step):
            ...
            rollout_reward = (g_loss+fake_loss).item()
            #chid_param = self.policy.get_parameters(generator)
        else:
            
            with torch.no_grad():
                for param in generator.parameters():
                    param.add_(torch.randn(param.size()).to(device) * 0.001)
                gen_imgs = generator(z)
                # Loss measures generator's ability to fool the discriminator
                g_loss = adversarial_loss(discriminator(gen_imgs), label_G.fill_(1.0))
                label_D = Variable(torch.LongTensor(imgs.shape[0]//pop_size),requires_grad=False).to(device)
    ...
    for i, (imgs, _) in enumerate(tqdm((dataloader))):
        ...
        for j in range(pop_size):
            ...
            results = [workers[j*fifth + f].do_rollouts(z,
                          parent_generator,  generator[j], discriminator, classifier, j,epoch) for f in range (fifth)]
        ...
        #results =  sorted(results, key=lambda x:x['rollout_reward'])
            ...
            children, pics  =  policy.get_best_pop(population, pictures, all_rollout_rewards)
            generator[j] = policy.set_parameters(children, generator[j])
            gen_imgs.append(pics)
        ...
        if batches_done % opt.sample_interval == 0:
            ...
            best_picture = [p[:5] for p in gen_imgs]
            best_picture = torch.cat(best_picture, dim=0)
            img_grid_fake = torchvision.utils.make_grid(best_picture, nrow=5,normalize=True)
